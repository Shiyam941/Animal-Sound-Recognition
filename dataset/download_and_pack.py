...
#        logs_dir = '_logs/download_dataset_minidata/{}'.format(get_filename(csv_path))
    logs_dir = get_filename(csv_path)
    ...
    for (n, line) in enumerate(lines):
        ...
        if len(video_paths) > 0:
            ...
            audio_path = "F:\\audioset_data\\unbalanced_train_segments\\" + audio_path
            # Extract audio in wav format
            os.system("ffmpeg -loglevel panic -i {} -y -ac 1 -ar 32000 -ss {} -t 00:00:{} {} "\
                .format(video_path, 
                str(datetime.timedelta(seconds=start_time)), duration,audio_path))
# -ac Set the number of audio channels. For output streams it is set by default to the number of 
# input audio channels. For input streams this option only makes sense for audio grabbing devices 
# and raw demuxers and is mapped to the corresponding demuxer options. 

# -ar Set the audio sampling frequency. For output streams it is set by default to the frequency 
...
#    logs_dir = '_logs/pack_waveforms_to_hdf5/{}{}'.format(prefix, get_filename(csv_path))
    logs_dir = get_filename(csv_path) + '_' + prefix
    create_folder(logs_dir)
    create_logging(logs_dir, filemode='w')
...
# For split_unbalanced_csv_to_partial_csvs function
args.unbalanced_csv = r"your_path\unbalanced_train_segments\unbalanced_train_segments_clean.csv"              
# type=str, required=True, help='Path of unbalanced_csv file to read.'
args.unbalanced_partial_csvs_dir = r"your_path\unbalanced_train_segments" #type=str, required=True, help='Directory to save out split unbalanced partial csv.')

split_unbalanced_csv_to_partial_csvs(args)

#For download_wavs function
...
args.audios_dir = r"unbalanced_train_data_xx"     #type=str, required=True, help='Directory to save out downloaded audio.')
args.mini_data=False     #action='store_true', default=True, help='Set true to only download 10 audios for debugging.')
